...
const getMap = (line) => {
  ...
  // get the quick numbers - 1, 4, 7, and 8
  t = tokens.find(t => t.length === 2)
  if (t) map[1] = t.sort()
  t = tokens.find(t => t.length === 3)
  if (t) map[7] = t.sort()
  t = tokens.find(t => t.length === 4)
  if (t) map[4] = t.sort()
  t = tokens.find(t => t.length === 7)
  if (t) map[8] = t.sort()
  // 2, 3, and 5
  tokens.filter(t => t.length === 5).forEach(t => {
    if (diff(t, map[4]).length === 3) {
      map[2] = t.sort()
    } else {
      if (diff(map[1], t).length === 0) {
        map[3] = t.sort()
      } else {
        map[5] = t.sort()
      }
  ...
  // 6, 9, and 0
  tokens.filter(t => t.length === 6).forEach(t => {
    if (diff(t, map[4]).length === 2) {
      map[9] = t.sort()
    } else {
      if (diff(map[1], t).length === 0) {
        map[0] = t.sort()
...
const getMapDigit = (map, value) => {
	return Object.keys(map).find(k => map[k] === value)
}

const getDigits = (map, line) => {
	let d = ''
  let tokens = line.trim().split(' ').forEach(t => {
  	t = t.sort()
    const v =  getMapDigit(map, t)
    d += v
  })
	return parseInt(d)
}
...
let total = 0
a.forEach(line => {
  const map = getMap(line.split(' | ')[0])
  const digits = getDigits(map, line.split(' | ')[1])
	total += digits
})
console.log(total)