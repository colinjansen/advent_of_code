...
def ex(line):
    ...
    accumulator_2 = reduce(lambda a, i: entries[i][0] - a, [i for i in range(len(entries) - 2, -1, -1)], 0)
    return accumulator_1, accumulator_2

with open("_input/day9.txt", encoding='utf8') as f:
    lines = f.read().splitlines()

total_1 = sum([ex(line)[0] for line in lines])
total_2 = sum([ex(line)[1] for line in lines])
