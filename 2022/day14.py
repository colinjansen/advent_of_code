...
"""
def get_bounding_box(cave):
    bbox = BoundingBox()
    for p in cave:
        if bbox.right == None or p[0] > bbox.right: bbox.right = p[0]
        if bbox.left == None or p[0] < bbox.left: bbox.left = p[0]
        if bbox.bottom == None or p[1] > bbox.bottom: bbox.bottom = p[1]
        if bbox.top == None or p[1] < bbox.top: bbox.top = p[1]
    return bbox
...
def expand_line(a, b):
    x_i = b[0] - a[0]
    y_i = b[1] - a[1]
    if x_i != 0: x_i //= abs(x_i)
    if y_i != 0: y_i //= abs(y_i)
    c = a
    line = [c]
    while c != b:
        c = (c[0] + x_i, c[1] + y_i)
...
"""
def get_starting_rocks(lines: list[Line]):
    spaces = {}
    for line in lines:
...
"""
def can_move(cave, bbox, s):
    # have we moved outside of the bbox?
    if s[0] < bbox.left or s[0] > bbox.right or s[1] > bbox.bottom: return False
    # fall straight down
    d = (s[0], s[1] + 1)
    if d not in cave: return d
    # down and left?
    dl = (s[0] - 1, s[1] + 1)
    if dl not in cave: return dl
    # down and right
    dr = (s[0] + 1, s[1] + 1)
    if dr not in cave: return dr
...
"""
def add_sand(cave, bbox):
    s = (500, 0)
    move = can_move(cave, bbox, s)
    while (move != False and move != None):
        s = move
        move = can_move(cave, bbox, s)
...
otherwise return the new coordinate
"""
def can_move_v2(cave, floor, s):
    # we are blocked
    if s in cave: return False
    # we are on the floor
    if s[1] + 1 == floor: return None
    # fall straight down
    d = (s[0], s[1] + 1)
    if d not in cave: return d
    # down and left?
